...
export default class EditVendorsDetails extends Component {
  constructor(props) {
    super(props);
    this.state = {
      report: "",
  ...
  async getVendersByCategory() {
    ...
    let url =
      ...
      "&cuisine_id=" +
      this.state.event_arr.cuisine_id +
      "&latitude=" +
      this.state.event_arr.latitude +
      "&longitude=" +
      this.state.event_arr.longitude +
      "&event_category_id=" +
      this.state.event_arr.category_id +
      "&type=" +
      this.state.event_arr.category_type +
      "&budget_amount=" +
      this.state.event_arr.budget;
  ...
  totalAmount = () => {
    let addedItem = this.state.addedItem;
    consolepro.consolelog("addedItem", addedItem);
    var totalAmount = 0;
    for (let i = 0; i < addedItem.length; i++) {
  ...
  continueButton() {
    ...
    // return false
    this.props.navigation.navigate("proposedEventVedorlistEdit", {
      addedItemArray: data,
      event_arr: this.state.event_arr,
      item: this.state.item,
    });
    // this.props.navigation.navigate('ProposedEventVendorListTab0', { 'addedItemArray': data })
  }
      ...
      <SafeAreaView style={styles.container}>
        ...
        {this.state.vendor_arr != "NA" ? (
          <FlatList
            ...
            contentContainerStyle={{ paddingBottom: (mobileH * 12) / 100 }}
            showsHorizontalScrollIndicator={false}

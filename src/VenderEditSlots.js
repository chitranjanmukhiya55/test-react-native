...
export default class VenderEditSlots extends Component {
  constructor(props) {
    super(props);
    this.state = {
      celander: 0,
  ...
  getSlots() {
    let data2 = this.state.item.availability_time;
    let data = this.state.allTimeSlots;
    consolepro.consolelog("data154", data);
    consolepro.consolelog("data2", data2);
    for (let i = 0; i < data2.length; i++) {
      var arr = {
        id: data.length + 1,
        start_data: data2[i].start_time,
        end_data: data2[i].end_time,
        availability_id: data2[i].availability_id,
        start_time_new: data2[i].start_time_new,
        end_time_new: data2[i].end_time_new,
      };
      data.push(arr);
    }
    consolepro.consolelog("54545455", data);
    this.setState({ allTimeSlots: data, showSlot: true });
    consolepro.consolelog("allTimeSlots", this.state.allTimeSlots);
  }
  ...
  settime = (res) => {
    ...
    for (let i = 0; i < data1.length; i++) {
      if (i == this.state.index) {
        ...
        data1[i].start_time_new = sendtime;
      }
    }
  ...
  onDateChange(date, type) {
    if (date != null) {
      ...
      this.setState({
        ...
        showSlot: true,
        // allTimeSlots: [{ id: 0, start_data: '', end_data: '' }],
      });
      if (type == "START_DATE") {
  ...
  async removeSlits(item, index) {
    if (item.start_data != "" && item.end_data != "") {
      let user_arr = await localStorage.getItemObject("user_arr");
      consolepro.consolelog("user_arr>", user_arr);
      let user_id = user_arr.user_id;
      let url = config.baseURL + "delete_availablity.php";
      var data = new FormData();
      data.append("user_id", user_id);
      data.append("availability_id", item.availability_id);
  ...
  async getDatesBetween() {
    let date = moment(this.state.item.availability_date).format("YYYY-MM-DD");
    let user_arr = await localStorage.getItemObject("user_arr");
    consolepro.consolelog("user_arr", user_arr);
    let user_id = user_arr.user_id;
    const dates = [];
    ...
    let url = config.baseURL + "edit_availablity.php";
    ...
    for (let i = 0; i < allTimeSlots.length; i++) {
      ...
      data.append("availability_id[]", allTimeSlots[i].availability_id);
      data.append("start_time[]", allTimeSlots[i].start_time_new);
      data.append("end_time[]", allTimeSlots[i].end_time_new);
    }
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          <CalendarPicker
            ...
            selectedStartDate={this.state.item.availability_date}
          />

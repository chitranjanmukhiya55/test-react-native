...
export default class VendorForgotOTPVerify extends Component {
  constructor(props) {
    super(props);
    this.state = {
      otpText: "",
  ...
  componentDidMount() {
    ...
    consolepro.consolelog("email", this.state.email);
  ...
  Resendotpbtn = async () => {
    if (config.app_status == 0) {
      ...
      Keyboard.dismiss();
      let url = config.baseURL + "forgot_password_resend_otp.php";
  ...
  Otpveryfication = async () => {
    if (config.app_status == 0) {
      ...
      let url = config.baseURL + "forgot_password_otp_verify.php";
      var data = new FormData();
      data.append("user_id", this.state.user_id);
      data.append("otp", otp);
      consolepro.consolelog("data", data);
      apifuntion
        ...
        .then((obj) => {
          ...
          if (obj.success == "true") {
            this.props.navigation.navigate("VendorCreatePassword", {
